...
exports.createSearch = function(key, fields, opts, cb){
  const
    ...
    if (err) {
      ...
      if (String(err).indexOf('Unknown Index name') > 0 ){
        ...
        fields.forEach(function(field) {
          //args.push(field, field === 'timestamp' ? 'numeric' : 'text');
          args.push(field);
        });
...
Query.prototype.end = function(fn){
  ...
  db.send_command(
    ...
    function(err,resp) {
      if (err) { fn(err); } else {
        // return full response from redis command
        fn(err,resp);
      }
    }
  );
